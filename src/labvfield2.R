...
px <- ggplot(all, aes(x=tissue, group=Type)) + 
  geom_density(aes(fill=Type), alpha=0.2)+
  scale_fill_grey() +
...
###plot by density, source (KDES, figure 2)
levels(all$Source)
unique(all$Source)
all$Source <- factor(all$Source, levels = c("Battaglin et al. 2016","Glinski 2013","Smalling et al. 2013", "Smalling et al. 2015", "Swanson et al. 2018","USGS 2018", "Lab"))
#check to make sure that the factors are in fact reorder
ps <- ggplot(all, aes(x=tissue, group=Source)) + 
  geom_density(aes(fill=Source), alpha=0.4)+
  scale_fill_manual(values=c("#E69F00", "#CC0000", "#56B4E9", "#009E73", "#CC79A7","#F0E442", "#000000"))+
  xlab('Log normalized tissue concentrations by study across all pesticides and species (ug/g)')+
  ylab('Density')+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.x = element_text(size = 12))
ps
...
ps + annotate('text', x = ps.text$x, y = ps.text$y,
              label = sprintf('n = %d', ps.text$n), vjust = -0.5, hjust =0.9, angle=10, size=4)
...
unique(test$chemical)#n34 at beginning, now 30 same number of records, we haven't dropped anything by accident
#fix ppddt 
all<-test
#fix naming
...
unique(alln$Source)
pp <- ggplot(alln, aes(x=chemical, y=tissue, fill=Source)) + 
  # geom_boxplot()+
